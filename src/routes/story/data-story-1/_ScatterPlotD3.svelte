<script lang="ts" context="module">
	export type ScatterPlotD3Data = { x: number; y: number; z: string }[];
	export type ScatterPlotD3Options = {
		xDomain?: number[];
		yDomain?: number[];
		showGrid?: boolean;
		pointRadius?: number;
...
<script lang="ts">
	...
	export let config: ChartProps<ScatterPlotD3Data> & ScatterPlotD3Options;
	...
	$: {
		root
			...
			.attr('fill', (d, i) => colorScale(d.z) as any)
			.attr('fill-opacity', 0.6)
			.attr('stroke', (d, i) => colorScale(d.z) as any)
			.attr(
				'data-tippy-content',
				(d) =>
					`<strong>${d.z}</strong><br>
